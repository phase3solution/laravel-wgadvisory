...
{{-- 
                @foreach ($assessments as $threat)
                    ...
                    @if ($threat->facilityRiskResult)
                        @php
                            array_push($avg,$threat->facilityRiskResult->average)
                        @endphp
                    @endif
...
--}}
                ...
                <tr>
                    <td class="text-center bold">
                        @if ($assessment->facilityRiskResult)
                            {{-- {{$char_arr[$assessment->facilityRiskResult->average]}} --}}
                            {{$char++}}
                        @else 
                            {{-- {{$char_arr[0]}} --}}
                            {{$char}}
                    ...
                    <td class="text-center bold color-four">
                        @if ($assessment->facilityRiskResult)
                            {{$assessment->facilityRiskResult->average}}
                        @else 
                            0
                        @endif
                    </td>  
