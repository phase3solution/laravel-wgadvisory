                ...
                <div class="">
                    ...
                    @if (count($assessment->children)> 0)
                        ...
                        @foreach ($assessment->children as $section)
                            ...
                            <form class="itManagementForm" method="POST">
                                @csrf
                                                ...
                                                <table class="table section-table table-bordered">
                                                    <tr>
                                                        ...
                                                        <td class="sub-title-txt bg-gray text-center">
                                                            ...
                                                            (<span class="totalValueS">@if ($table->itManagementResult) {{$table->itManagementResult->total}} @else 0 @endif</span> out of <span>{{count($table->children)*5}}</span> / Avg: <span class="averageValueS">@if ($table->itManagementResult) {{$table->itManagementResult->average}} @else 0 @endif</span> )
                                                            @else
                                                            (<span class="totalValueS">@if ($table->itManagementResult) {{$table->itManagementResult->total}} @else 0 @endif</span> out of 0 / Avg: <span class="averageValueS">@if ($table->itManagementResult) {{$table->itManagementResult->average}} @else 0 @endif</span> )
                                                            @endif
                                                           
                                                            @if ($table->itManagementResult)
                                                            <input type="hidden" name="total[{{$table->id}}]" class="totalValueI" value="{{$table->itManagementResult->total}}">
                                                            <input type="hidden" name="average[{{$table->id}}]" class="averageValueI" value="{{$table->itManagementResult->average}}">
                                                    ...
                                                    @if (count($table->children)>0)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="p-0 @if ($question->itManagementResult) c-{{$question->itManagementResult->answer}} @else c-0 @endif ">
                                                                <select name="answer[{{$question->id}}]" class="form-control text-16 transparent rating_variable @if ($question->itManagementResult) c-{{$question->itManagementResult->answer}} @else c-0 @endif">
                                                                  
                                                                  @if ($question->itManagementResult)

                                                                  <option value="0" @if($question->itManagementResult->answer == 0) selected @endif >N\A</option>
                                                                  <option value="1" @if($question->itManagementResult->answer == 1) selected @endif >Initial</option>
                                                                  <option value="2" @if($question->itManagementResult->answer == 2) selected @endif >Managed</option>
                                                                  <option value="3" @if($question->itManagementResult->answer == 3) selected @endif >Defined</option>
                                                                  <option value="4" @if($question->itManagementResult->answer == 4) selected @endif >Measured</option>
                                                                  <option value="5" @if($question->itManagementResult->answer == 5) selected @endif >Optimizing</option>
                                                            ...
                                                            <td class="p-0">
                                                                ...
                                                                <textarea name="comment[{{$question->id}}]" class="form-control text-16 bold comment" cols="10" rows="1">@if($question->itManagementResult){{$question->itManagementResult->comment}}@endif</textarea>
                                                            </td>
                                                        </tr>
                                                        @endforeach
...
<script>
    $(document).ready(function(){
        ...
        $('.itManagementForm').on('submit', function(e){

                e.preventDefault();
                var formData = $(this).serialize();
                $.ajax({
                    type: "POST",
                    url: "{{ url('it-management-assessment-result-store') }}",
                    data: formData,
    ...
    function resetAssessment(company_id) {
            ...
            $.ajax({
                type: "GET",
                url: "{{ url('it-management-assessment-result-reset') }}/" + company_id,
                success: function(response) {
                    if (response.status) {
    ...
    function publishAssessment(assessment_id) {
        $.ajax({
            type: "GET",
            url: "{{ url('it-management-assessment-result-publish') }}/" + assessment_id,
            success: function(response) {
                if (response.status) {
