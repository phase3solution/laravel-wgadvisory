            ...
            <div class="container-fluid">
                <div class="card mt-1">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-5">
                                <div class="thum"><img src="{{asset('frontend/assets')}}/img/dr-readiness.jpeg" alt=""></div>
                            </div>
                            <div class="col-sm-7">
                                <div class="graph_item_maturity purple">
                                    ...
                                    <div class="wrap">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-5">
                                                <div class="thum">
                                                    <div class="thum"><img src="<?php echo''.$home_url.'';?>/img/icon/icon-business_solutions.png" alt=""></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="">
                    ...
                    @if (count($assessment->children)> 0)
                        ...
                        @foreach ($assessment->children as $section)
                            ...
                            <form class="drmForm" method="POST">
                                @csrf
                                                ...
                                                <table class="table section-table table-bordered">
                                                    <tr>
                                                        ...
                                                        <td class="sub-title-txt bg-gray text-center">
                                                            ...
                                                            (<span class="totalValueS">@if ($table->drmResult) {{$table->drmResult->total}} @else 0 @endif</span> out of <span>{{count($table->children)*5}}</span> / Avg: <span class="averageValueS">@if ($table->drmResult) {{$table->drmResult->average}} @else 0 @endif</span> )
                                                            @else
                                                            (<span class="totalValueS">@if ($table->drmResult) {{$table->drmResult->total}} @else 0 @endif</span> out of 0 / Avg: <span class="averageValueS">@if ($table->drmResult) {{$table->drmResult->average}} @else 0 @endif</span> )
                                                            @endif
                                                           
                                                            @if ($table->drmResult)
                                                            <input type="hidden" name="total[{{$table->id}}]" class="totalValueI" value="{{$table->drmResult->total}}">
                                                            <input type="hidden" name="average[{{$table->id}}]" class="averageValueI" value="{{$table->drmResult->average}}">
                                                        ...
                                                        <td class="sub-title-txt bg-gray text-center">Comments</td>
                                                    </tr>
                                                    ...
                                                    @if (count($table->children)>0)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="p-0 bg-light-gray @if ($question->drmResult) c-{{$question->drmResult->answer}} @else c-0 @endif ">
                                                                <select name="answer[{{$question->id}}]" class="form-control text-16 transparent rating_variable">
                                                                  
                                                                  @if ($question->drmResult)

                                                                  <option value="0" @if($question->drmResult->answer == 0) selected @endif >N\A</option>
                                                                  <option value="1" @if($question->drmResult->answer == 1) selected @endif >Initial</option>
                                                                  <option value="2" @if($question->drmResult->answer == 2) selected @endif >Managed</option>
                                                                  <option value="3" @if($question->drmResult->answer == 3) selected @endif >Defined</option>
                                                                  <option value="4" @if($question->drmResult->answer == 4) selected @endif >Measured</option>
                                                                  <option value="5" @if($question->drmResult->answer == 5) selected @endif >Optimizing</option>
                                                            ...
                                                            <td class="p-0">
                                                                ...
                                                                <textarea name="comment[{{$question->id}}]" class="form-control text-16 bold comment" cols="10" rows="1">@if($question->drmResult){{$question->drmResult->comment}}@endif</textarea>
                                                            </td>
                                                        </tr>
                                                        @endforeach
