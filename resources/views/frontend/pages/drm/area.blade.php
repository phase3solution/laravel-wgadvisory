                                                ...
                                                <table class="table section-table table-bordered">
                                                    ...
                                                    @if (count($table->children)>0)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="p-0 @if ($question->drmResult) c-{{$question->drmResult->answer}} @else c-0 @endif ">
                                                                <select name="answer[{{$question->id}}]" class="form-control text-16 transparent rating_variable @if ($question->drmResult) c-{{$question->drmResult->answer}} @else c-0 @endif">
...
<script>
    $(document).ready(function(){
        ...
        $('.drmForm').on('submit', function(e){

                e.preventDefault();
                var formData = $(this).serialize();
                $.ajax({
                    type: "POST",
                    url: "{{ url('drm-assessment-result-store') }}",
                    data: formData,
    ...
    function resetAssessment(company_id) {
            ...
            $.ajax({
                type: "GET",
                url: "{{ url('drm-assessment-result-reset') }}/" + company_id,
                success: function(response) {
                    if (response.status) {
    ...
    function publishAssessment(assessment_id) {
        $.ajax({
            type: "GET",
            url: "{{ url('drm-assessment-result-publish') }}/" + assessment_id,
            success: function(response) {
                if (response.status) {
